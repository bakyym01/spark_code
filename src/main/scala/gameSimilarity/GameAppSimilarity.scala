...
case class UserPkg(var uptime: Long = 0, var pkgName: String = "")
...
//        conf.set("spark.executor.memory", "10g")
    ...
    /**
      * 本地
      */
//      val orcPath = "F:/spark_input/000399_0"
    val orcPath = args(0)
...
//        val hql: String = "select pkgname,count(*) n from " + "(select a.aid,a.pkgname from (select _col0 as aid,_col1 as pkgname from sum_install_daily_14d where _col4 REGEXP \'^game_\') a " + "left join " + "(select c.aid from (select _col0 as aid,count(*) n from sum_install_daily_14d where _col4 REGEXP \'^game_\' group by _col0) c where c.n>50) b " + "on a.aid=b.aid where b.aid is null) d group by pkgname"
    ...
    val filterRDD: RDD[(String,String)] = df.rdd.filter(r => {
      ...
      r.foreach(f=>{
        aid=f.getString(0)
        pkgName = f.getString(1)
        uptime = f.getLong(2).toString
        list += ((aid,s"${pkgName}\001${uptime}"))
      })
      list.toIterator
    }).reduceByKey((x,y)=>{x+"\002"+y})


    val templeRdd: RDD[(String, Double)] = filterRDD.filter(f => {
      //先将pair的数据按aid  groupby之后，再过滤掉安装过大于50个应用的人，因为这种人的要么是BUG ID产生的，要么就是兴趣太广泛对于物品相似度来讲不具备什么参考价值
      val strings = f._2.split("\002")
      val size: Int = strings.size
      if (size > 50) {
        false
      } else {
        true
      ...
      val tlist = new ArrayBuffer[(String, Double)]
      f.foreach(x => {
        val list = x._2.split("\002")
        val size: Int = list.size
        val sd: Double = 1 / Math.log10(1 + size)
        for (i <- 0 until size - 1) {
          val pkgName = list(i).split("\001")(0)
          val uptime = list(i).split("\001")(1).toLong
          for (a <- i + 1 until size) {
            val pkgName1 = list(a).split("\001")(0)
            val uptime1 = list(a).split("\001")(1).toLong
            val td: Double = 1.0 / (1 + Math.abs(uptime - uptime1))
            val value: Double = (sd * td)
            val key: String = if (pkgName.compareTo(pkgName) < 0) {
              s"${pkgName}\t${pkgName1}"
            } else {
              s"${pkgName1}\t${pkgName}"
            }
...
//    })
    ...
    val simiRDD: RDD[Row] = templeRdd.filter(f => {
...
//    val outSql = "select concat(d.pkgname1,'\t',d.pkgname2,'\t',d.values,'\t',d.rn) from pkg_rank d inner join pkg_distinct c on d.pkgname1 = c.pkgname1"
    ...
    /**
      * 本地
      */
//        outDF.write.mode(SaveMode.Overwrite).format("text").save("F:/spark_output/spark_pj4/gameappsimilarity1")
